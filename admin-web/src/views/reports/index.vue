<template>
  <div class="reports-page">
    <div class="header">
      <h2>报告管理</h2>
      <el-button type="primary" @click="handleAdd">录入报告</el-button>
    </div>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑报告' : '录入报告'" width="560px">
...
const onAppointmentSearch = (keyword) => {
  loadAppointmentOptions(keyword || '')
}
...
const handleSubmit = async () => {
  ...
  if (form.value.id) {
    await request.patch(`/reports/${form.value.id}/`, payload)
  } else {
...
const handleUpload = async ({ file }) => {
  ...
  const data = await request.post('/users/upload/', formData, {
